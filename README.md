...
XML responses reference XSLT files under the URI prefix specified by `WEBZIP_XSLTPFX`, by suffixing `not-found`, `bad-method` or `dir-list`, and then the suffix specified by `WEBZIP_XSLTSFX` (or `.xsl` by default).
